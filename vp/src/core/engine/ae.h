...
class AE : public sc_core::sc_module {
   public:
    ...
    void decode_execute(Fileds& fileds) {
        ...
        compute(fileds, in_data_fp32, p_data_fp32, fileds["smx.n"], fileds["smx.m"], fileds["smx.dim"], fileds["act.n"], fileds["act.m"]);
    ...
    void compute(Fileds& fileds, float* in_data, float* p_data, int smx_n, int smx_m, int smx_dim, int act_n, int act_m) {
        uint32_t opcode = fileds["opcode"];
