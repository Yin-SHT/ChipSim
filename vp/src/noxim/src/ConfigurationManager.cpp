...
void loadConfiguration() {
	...
	try {
		...
		cout << " Failed" << endl;
		cerr << "ERROR at line " << pe.mark.line + 1 << " column " << pe.mark.column + 1 << ": " << pe.msg
		     << ". Please check identation." << endl;
		exit(0);
	}
	...
	cout << "Loading PE configurations from file \"" << GlobalParams::pe_config_filename << "\"...";
	try {
		pe_config = YAML::LoadFile(GlobalParams::pe_config_filename);
		cout << " Done" << endl;
	} catch (YAML::BadFile &e) {
		cout << " Failed" << endl;
		cerr << "The specified YAML PE configurations file was not found!" << endl;
		exit(0);
	} catch (YAML::ParserException &pe) {
		cout << " Failed" << endl;
		cerr << "ERROR at line " << pe.mark.line + 1 << " column " << pe.mark.column + 1 << ": " << pe.msg
	...
	GlobalParams::tlm_global_quantum = readParam<unsigned int>(pe_config, "tlm_global_quantum", 10);
	GlobalParams::use_instr_dmi = readParam<bool>(pe_config, "use_instr_dmi", false);
	GlobalParams::use_data_dmi = readParam<bool>(pe_config, "use_data_dmi", false);
	GlobalParams::mem_size = readParam<addr_t>(pe_config, "mem_size", 1024 * 1024 * 32);
	GlobalParams::mem_start_addr = readParam<addr_t>(pe_config, "mem_start_addr", 0x00000000);
	GlobalParams::mem_end_addr = readParam<addr_t>(pe_config, "mem_end_addr", 0x02000000);
	GlobalParams::clint_start_addr = readParam<addr_t>(pe_config, "clint_start_addr", 0x02000000);
	...
	GlobalParams::noc_trace_mode = readParam<bool>(config, "noc_trace_mode");
	...
	if (GlobalParams::topology == TOPOLOGY_BASELINE || GlobalParams::topology == TOPOLOGY_BUTTERFLY ||
	...
	for (YAML::const_iterator hubs_it = config["Hubs"].begin(); hubs_it != config["Hubs"].end(); ++hubs_it) {
		...
		copy(GlobalParams::hub_configuration[hub_id].rxChannels.begin(),
		     GlobalParams::hub_configuration[hub_id].rxChannels.end(), inserter(channelSet, channelSet.end()));
		copy(GlobalParams::hub_configuration[hub_id].txChannels.begin(),
		     GlobalParams::hub_configuration[hub_id].txChannels.end(), inserter(channelSet, channelSet.end()));
	}
...
void showHelp(char selfname[]) {
	cout << "Usage: " << selfname << " [options]" << endl
	     ...
	     << "\t-pir R TYPE\t\tSet the packet injection rate R [0..1] and the time distribution TYPE where TYPE is one "
	        "of the following:"
	     << endl
	     ...
	     << "\t\tlocal L\t\tRandom traffic with a fraction L (0..1) of packets having a destination connected to the "
	        "local hub, i.e. not using wireless"
	     << endl
	     ...
	     << "\t-volume N\t\tStop the simulation when either the maximum number of cycles has been reached or N flits "
	        "have"
	     << endl
	     << "\t\t\t\tbeen delivered" << endl
	     ...
	     << endl
	     << "If you find this program useful please don't forget to mention in your paper Maurizio Palesi "
	        "<maurizio.palesi@unikore.it>"
	     << endl
	     << "If you find this program useless please feel free to complain with Davide Patti "
	     ...
	     << "and if need to solve any other problem of your life please contact Turi Monteleone "
	        "<salvatore.monteleone@dieei.unict.it>"
	     << endl;
}

void showConfig() {
	cout << "Using the following configuration: " << endl
         << "- elf = " << GlobalParams::elf << endl
         << "- intercept_syscalls = " << GlobalParams::intercept_syscalls << endl
         << "- use_debug_runner = " << GlobalParams::use_debug_runner << endl
         << "- debug_port = " << GlobalParams::debug_port << endl
         << "- pe_trace_mode = " << GlobalParams::pe_trace_mode << endl
         << "- tlm_global_quantum = " << GlobalParams::tlm_global_quantum << endl
         << "- use_instr_dmi = " << GlobalParams::use_instr_dmi << endl
         << "- use_data_dmi = " << GlobalParams::use_data_dmi << endl
         << "- mem_size = " << hex << "0x" << GlobalParams::mem_size << endl
         << "- mem_start_addr = " << hex << "0x" << GlobalParams::mem_start_addr << endl
         << "- mem_end_addr = " << hex << "0x" << GlobalParams::mem_end_addr << endl
         << "- clint_start_addr = " << hex << "0x" << GlobalParams::clint_start_addr << endl
         << "- clint_end_addr = " << hex << "0x" << GlobalParams::clint_end_addr << endl
         << "- sys_start_addr = " << hex << "0x" << GlobalParams::sys_start_addr << endl
         << "- sys_end_addr = " << hex << "0x" << GlobalParams::sys_end_addr << endl << dec
         << "- quiet = " << GlobalParams::quiet << endl
         << "- use_E_base_isa = " << GlobalParams::use_E_base_isa << endl
         << "- verbose_mode = " << GlobalParams::verbose_mode << endl
	     << "- noc_trace_mode = " << GlobalParams::noc_trace_mode
	     << endl
	...
	for (unsigned int i = 0; i < GlobalParams::hotspots.size(); i++) {
		if (GlobalParams::topology == TOPOLOGY_MESH) {
			if (GlobalParams::hotspots[i].first >= GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y) {
				cerr << "Error: hotspot node " << GlobalParams::hotspots[i].first << " is invalid (out of range)"
				     << endl;
				exit(1);
			}
		} else {
...
void parseCmdLine(int arg_num, char *arg_vet[]) {
	if (arg_num == 1)
		...
		for (int i = 1; i < arg_num; i++) {
			...
			else if (!strcmp(arg_vet[i], "-config") || !strcmp(arg_vet[i], "-power") || !strcmp(arg_vet[i], "-pe"))
...
void configure(int arg_num, char *arg_vet[]) {
	...
	for (int i = 1; i < arg_num; i++) {
		if (!strcmp(arg_vet[i], "-pe")) {
			GlobalParams::pe_config_filename = arg_vet[++i];
			pe_config_found = true;
			break;
		}
	}

	if (!pe_config_found) {
		std::ifstream infile(PE_CONFIG_FILENAME);
		if (infile.good())
			GlobalParams::pe_config_filename = PE_CONFIG_FILENAME;
		else {
			cerr << "No YAML PE configurations file found!\n Use -pe to load examples from config_examples folder"
			     << endl;
			exit(0);
		}
	}
