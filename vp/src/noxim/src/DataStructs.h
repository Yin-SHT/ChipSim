...
// Header between DMACTRL and Router
struct Header {
...
struct Packet {
	...
	void make(const int s, const int d, const int vc, const double ts, const int sz, struct Header &header) {
		src_id = s;
		dst_id = d;
		vc_id = vc;
		timestamp = ts;
        ...
        if (header.data != nullptr) {
            data = new uint8_t[header.len];
            memcpy(data, header.data, header.len);
        } else {
            data = nullptr;
        }
	}
...
struct NoP_data {
	...
	inline bool operator==(const NoP_data &nop_data) const {
		return (sender_id == nop_data.sender_id && nop_data.channel_status_neighbor[0] == channel_status_neighbor[0] &&
...
struct TBufferFullStatus {
	TBufferFullStatus() {
		for (int i = 0; i < MAX_VIRTUAL_CHANNELS; i++) mask[i] = false;
	};
