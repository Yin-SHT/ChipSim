...
void Router::rxProcess() {
	if (reset.read()) {
		...
		for (int i = 0; i < DIRECTIONS + 2; i++) {
			...
			TBufferFullStatus bfs;
			for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) bfs.mask[vc] = buffer[i][vc].IsFull();
			buffer_full_status_rx[i].write(bfs);
		}
	}
}
			...
			for (int k = 0; k < GlobalParams::n_virtual_channels; k++) {
				...
				if (!buffer[i][vc].IsEmpty()) {
					...
					if (flit.flit_type == FLIT_TYPE_HEAD) {
						...
						LOG << " checking availability of Output[" << o << "] for Input[" << i << "][" << vc
						    << "] flit " << flit << endl;
...
void Router::perCycleUpdate() {
	if (reset.read()) {
		for (int i = 0; i < DIRECTIONS + 1; i++) free_slots[i].write(buffer[i][DEFAULT_VC].GetMaxBufferSize());
	} else {
...
vector<int> Router::routingFunction(const RouteData &route_data) {
	if (GlobalParams::use_winoc) {
		...
		// - NOTE: default threshold is 0, which means I=D, i.e., we explicitly ask the destination D to be connected to
		// the target radio hub
		if (hasRadioHub(local_id)) {
			// Check if destination is directly connected to an hub
			if (hasRadioHub(route_data.dst_id) && !sameRadioHub(local_id, route_data.dst_id)) {
				...
				if (connectedHubs(it1->second, it2->second)) {
					LOG << "Destination node " << route_data.dst_id << " is directly connected to a reachable RadioHub"
					    << endl;
					vector<int> dirv;
			...
			if (GlobalParams::winoc_dst_hops > 0) {
				...
				for (int i = 1; i <= GlobalParams::winoc_dst_hops; i++) {
					...
					if (hasRadioHub(candidate_hop) && !sameRadioHub(local_id, candidate_hop)) {
						...
						LOG << "Relaying to hub-connected node " << candidate_hop << " to reach destination "
...
int Router::NoPScore(const NoP_data &nop_data, const vector<int> &nop_channels) const {
	int score = 0;
...
void Router::configure(const int _id, const double _warm_up_time, const unsigned int _max_buffer_size,
...
void Router::ShowBuffersStats(std::ostream &out) {
	for (int i = 0; i < DIRECTIONS + 2; i++)
		for (int vc = 0; vc < GlobalParams::n_virtual_channels; vc++) buffer[i][vc].ShowStats(out);
}
