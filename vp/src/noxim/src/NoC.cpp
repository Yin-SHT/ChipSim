...
void NoC::buildCommon() {
	...
	for (map<int, HubConfig>::iterator it = GlobalParams::hub_configuration.begin();
		...
		// for (map<int, int>::iterator it1 = GlobalParams::hub_for_tile.begin(); it1 !=
		// GlobalParams::hub_for_tile.end(); it1++ ) LOG<<"it1 first "<< it1->first<< "second"<< it1->second<<endl;

		// Determine, from configuration file, which Hub is connected to which Channel
		for (vector<int>::iterator iit = hub_config.txChannels.begin(); iit != hub_config.txChannels.end(); ++iit) {
			int channel_id = *iit;
		...
		for (vector<int>::iterator iit = hub_config.rxChannels.begin(); iit != hub_config.rxChannels.end(); ++iit) {
			int channel_id = *iit;
		...
		hub[hub_id]->power.configureHub(
		    GlobalParams::flit_size, GlobalParams::hub_configuration[hub_id].toTileBufferSize,
		    GlobalParams::hub_configuration[hub_id].fromTileBufferSize, GlobalParams::flit_size,
		    GlobalParams::hub_configuration[hub_id].rxBufferSize, GlobalParams::hub_configuration[hub_id].txBufferSize,
...
void NoC::buildButterfly() {
	...
	for (int j = 0; j < dimY; j++) {
		for (int i = 0; i < dimX; i++) {
			...
			sprintf(
			    tile_name, "Switch[%d][%d]_(#%d)", i, j,
			    tile_id);  // cout<<"tile_name=" <<tile_name<< " i=" <<i << " j=" << j<< " tile_id "<< tile_id<< endl;
			t[i][j] = new Tile(tile_name, tile_id);

			// cout << "switch  " << i <<  " " << j << "   has an Id = " << tile_id <<  endl;
			//  Tell to the router its coordinates
			t[i][j]->r->configure(tile_id, GlobalParams::stats_warm_up_time, GlobalParams::buffer_depth, grtable);
			t[i][j]->r->power.configureRouter(GlobalParams::flit_size, GlobalParams::buffer_depth,
	...
	for (int i = 0; i < n; i++) {
		...
		core[i]->r->configure(core_id, GlobalParams::stats_warm_up_time, GlobalParams::buffer_depth, grtable);
		core[i]->r->power.configureRouter(GlobalParams::flit_size, GlobalParams::buffer_depth, GlobalParams::flit_size,
...
void NoC::buildMesh() {
	...
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
		for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
			...
			// Tell to the router its coordinates
			t[i][j]->r->configure(j * GlobalParams::mesh_dim_x + i, GlobalParams::stats_warm_up_time,
		...
		for (int s = 0; s < 3; s++) {
			for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
				if (s == 0)
					...
					std::printf("|%d   %d", t[i][j]->r->buffer[s][0].getCurrentFreeSlots(),
